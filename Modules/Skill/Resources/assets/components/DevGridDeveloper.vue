    ...
    const gridDataSource = {
        store: new CustomStore({
            load: (loadOptions) => {
                ...
                return fetch(`${url}/developers${params}`)
                    .then(handleErrors)
            ...
            insert: (values) => {
                return axios.post(`${url}/developers`, values, {method: "POST"});//.then());
            },
            remove: (key) => {
                return axios.delete(`${url}/developers/` + encodeURIComponent(key.id), { method: "DELETE"  });//.then(handleErrors);
            },
            update: (key, values) => {
                return axios.put(`${url}/developers/` + encodeURIComponent(key.id),dataToUpdate(key, values),{ method: "DELETE"  });//.then(handleErrors);
            }
        })
    };
    export default {
        name: "DevGridDeveloper",
        components: {
            DxSwitch,
            DxDataGrid,
            DxEditing,

    ...
    const gridDataSource = {
        store: new CustomStore({
            load: (loadOptions) => {
                ...
                return fetch(`/api/skill/language${params}`)
                    .then(handleErrors)
            ...
            insert: (values) => {
                return axios.post(`/api/skill/language`, values, {method: "POST"});//.then());
            },
            remove: (key) => {
                return axios.delete(`/api/skill/language/` + encodeURIComponent(key.id), { method: "DELETE"  });//.then(handleErrors);
            },
            update: (key, values) => {
                return axios.put(`/api/skill/language/` + encodeURIComponent(key.id), values,{ method: "PUT"  });//.then(handleErrors);
            }
        })
    };
    export default {

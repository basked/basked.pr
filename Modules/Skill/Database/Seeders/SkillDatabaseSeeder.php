...
class SkillDatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        DB::table('sk_resource_assignments')->truncate();
//        DB::table('sk_tasks')->truncate();
//        DB::table('sk_dependencies')->truncate();
//        DB::table('sk_resources')->truncate();
        ...
        for ($i = 0; $i < sizeof($dependencies); $i++) {
            DB::table('sk_dependencies')->insert([
                'id' => $dependencies[$i]['id'],
                'predecessorId' => $dependencies[$i]['predecessorId'],
                'successorId' => $dependencies[$i]['successorId'],
                'type' => $dependencies[$i]['type'],
            ]);
        }
        // add resources
        for ($i = 0; $i < sizeof($resources); $i++) {
            DB::table('sk_resources')->insert([
                'id' => $resources[$i]['id'],
                'text' => $resources[$i]['text'],
            ]);
        }
        // add  resourceAssignments
        for ($i = 0; $i < sizeof($resourceAssignments); $i++) {
            DB::table('sk_resource_assignments')->insert([
                'id' => $resourceAssignments[$i]['id'],
                'task_id' => $resourceAssignments[$i]['taskId'],
                'resource_id' => $resourceAssignments[$i]['resourceId'],
            ]);
        }
